...
func attachWebFlags() func() workclient.Config {
	...
	var statsdPrefix = flag.String("statsd_prefix", "", "statsd prefix for the usda")

	var graphiteAddr = flag.String("graphite_addr", "", "graphite address for the usda metrics")
	var graphitePrefix = flag.String("graphite_prefix", "", "graphite prefix for the usda")

	var stdErrLog = flag.String("stderr_logfile", "", "writes all stderr log output to the given file or endpoint")

	var serviceName = flag.String("service_name", "usda", "name of the service that this configuration is running")
